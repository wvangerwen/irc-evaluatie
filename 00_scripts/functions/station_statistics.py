...
class IrcStats:
    ...
    def __init__(self, stationstats, irc_type):
        ...
        self.gauge = stationstats.df["station"]
        self.irc = stationstats.df[irc_type]
        self.residuals = self.irc - self.gauge
        self.gauge_mean = self.gauge.mean()
        self.irc_mean = self.irc.mean()
        self.BiasTotal = self.residuals.mean()
        self.gauge_cumu = self.gauge.cumsum().iloc[-1]
        ...
        if self.gauge_mean == 0:
            ...
            self.CV = round(self.gauge.std() / self.gauge_mean, 2)
            self.RelBiasTotal = round(
                self.BiasTotal / self.gauge_mean * 100, 1
            )  # Relative bias %.

        self.RelBiasTotalCumu = round(self.BiasTotalCumu / self.gauge_cumu * 100, 1)
...
class StationStats:
    ...
    def __init__(self, station):
        ...
        self.df_yesdata = (
            self.get_df_yesdata()
        )  # Filter table when True, we can use the value.
    ...
    def plot_scatter_ax(self, irc_type, ax):
        scatter = ax.scatter(
            x=self.df["station"], y=self.df[irc_type], c=self.df["class"]
        )

        (
            handles,
            labels,
        ) = scatter.legend_elements()  # TODO add number of features in certain class.
        labels = [
            self.classes.set_index("value").loc[int(re.findall(r"[0-9]+", i)[0])][
                "legend"
            ]
            for i in labels
        ]  # Get legend label from dataframe based on the class.
